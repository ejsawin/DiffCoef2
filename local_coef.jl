function local_coef(rr,EE,LL)
    ...
    vp_calc=midpoint(x->vp_int(x),log(vmin),log(v_step),int_steps)
    vp1_calc=midpoint(x->vp_int1(x),log(vmin),log(v_step),int_steps)

    vpsq_calc=midpoint(x->vpsq_int(x),log(vmin),log(v_step),int_steps)
    vpsq1_calc=midpoint(x->vpsq_int1(x),log(v_step),log(vmax),int_steps)
